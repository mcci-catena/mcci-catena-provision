...
##############################################################################
# 
# Module: mcci-catena-provision.py
#
# Function:
...
def ttncomms(**dVarArgs):
    ...
    Args: 
        **dVarArgs: ttnctl config info in dict
    ...
    if dVarArgs['HANDLERID']:
        dVarArgs['HANDLERID'] = '--handler-id=' + dVarArgs['HANDLERID'].replace('\n', '')
    ...
    sysEUI = sysEUI.replace('\n', '')
    devInfoCmdList.append(dVarArgs['BASENAME'] + sysEUI.lower())
...
if __name__ == '__main__':
    ...
    ttnctlCli = [True for dirfile in listDirContent if dirfile == 'ttnctl' or dirfile == 'ttnctl.exe']

    if not ttnctlCli:
        oAppContext.fatal("ttnctl not found; add to path: {}".format(pDir)) 

    if oAppContext.fRegister:
        ttncommResult = ttncomms(**oAppContext.dVariables)
